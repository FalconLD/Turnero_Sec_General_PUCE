                            ...
                            @forelse($payments as $payment)
                                ...
                                <tr @if($payment->status == 'pending') class="table-warning" @endif>
                                    ...
                                    <td>
                                        ...
                                        @php
                                            $comprobanteDataUri = "data:{$payment->comprobante_mime};base64,{$payment->comprobante_base64}";
                                        @endphp

                                        <button class="btn btn-xs btn-info" 
                                                data-toggle="modal" 
                                                data-target="#comprobanteModal" 
                                                data-src="{{ $comprobanteDataUri }}"
                                                data-student="{{ $payment->studentRegistration->names ?? 'Estudiante' }}">
                                            Ver Comprobante
                                        </button>

                                        @if(stripos($payment->comprobante_mime, 'pdf') === false)
...
$(document).ready(function() {
    ...
    if (src.toLowerCase().startsWith('data:application/pdf')) {
